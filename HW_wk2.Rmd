...
#OLD CODE
#state_avg <- grouped_studentsby_county %>% 
#  mutate(state_average = sum(passed)/sum(TotalStudents))
#  dplyr::select(County, TestSubject, GradeLevel, StudentGroup, StudentGroupType, state_average, #normalised_stateavg) %>%
#  distinct()
#why is the normalised value 1 for all of them?
```
...
compare_stateavg2 <- grouped_studentsby_county %>% 
  ...
  mutate(., WAcompare = case_when(county_avg >= 0.474 ~ str_c("equal or above WA average by", WAdiff, "percent", sep=" "), TRUE ~ str_c("below below the state average by", WAdiff, "percent", sep=" ")))
...
library(tmap)
qtm(ReportCardData_Map, fill = "WAdiff", fill.palette="-Blues")
